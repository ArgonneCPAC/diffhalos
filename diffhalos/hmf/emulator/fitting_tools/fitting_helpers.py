...
def jax_adam_wrapper(
    ...
    mMust accept inputs (params, data) and return a scalar and its gradients
    ...
    n_warmup: int, optional
        number of warmup iterationsl At the end of the warmup,
        the best-fit parameters are used as input parameters
        to the final burn

    warmup_n_step: int, optional
        number of Adam steps to take during warmup

    warmup_step_size: float, optional
        step size to use during warmup phase

    step_size: float, optional
        step size parameter in the Adam algorithm

    Returns
    -------
    params_step_n: ndarray of shape (n_params, )
        best-fit value of the parameters after n_step steps

    loss: float
        final value of the loss

    loss_arr: ndarray of shape (n_step, )
        value of the loss at each step

    params_arr: ndarray of shape (n_step, n_params)
        value of the model params at each step

    fit_terminates: int
        0 if NaN or inf is encountered by the fitter,
        causing termination before n_step,
