...
from .. import hmf_model_hlx as hmf_model


def test_cuml_hmf_evaluations():
    lgmp_arr = np.linspace(7.0, 15.0, 500)
    redshift = 0.2
    delta_c = 200.0

    res = hmf_model.predict_cuml_hmf(
        hmf_model.DEFAULT_COSMOLOGY_JAXCOSMO,
        lgmp_arr,
        redshift,
        delta_c,
    )
...
def test_get_mean_nhalos_from_volume_and_from_sky_area():
    ...
    redshifts = np.linspace(0.1, 2.0, 5)
    ...
    for redshift in redshifts:
        ...
        assert np.isfinite(nhalos_from_vol)
        assert np.isfinite(nhalos_from_area)
