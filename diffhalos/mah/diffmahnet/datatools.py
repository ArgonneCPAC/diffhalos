...
def load_all_tdata(path, is_test: bool | str = False, is_cens=True):
    ...
    fileinfo = list(zip(tdata_files, lgm_vals.tolist(),
                    t_vals.tolist(), is_cens_vals.tolist()))
    ...
    x_unbound = jnp.array([*get_unbounded_mah_params(
        DEFAULT_MAH_PARAMS._make(x.T))]).T
...
class DataHolder:
    ...
    def get_tgrid_and_log_mah(self, randkey, t_min=0.1, n_tgrid=20):
        ...
        tgrid = diffmahnet.gen_time_grids(randkey, self.t_obs, t_min=t_min,
                                          n_tgrid=n_tgrid)
